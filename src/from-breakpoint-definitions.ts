...
const mediaQueryFromBreakpoint = ({ min, max }: BreakpointDefinition) =>
  [
    ["min", min],
    ["max", max],
  ]
    ...
    .map(
      ([string_, value]) =>
        `(${string_}-width: ${typeof value === "string" ? value : `${String(value)}px`})`,
    )
...
const fromBreakpointDefinitions = <K>(
  ...
  merge(
    ...Object.entries(breakpointDefinitions).map(
      ([name, breakpoint]): Observable<{
        name: K
        matches: boolean
      }> => {
        const mediaQueryList = matchMedia(mediaQueryFromBreakpoint(breakpoint))
